...
class Interpreter(object):
    ...
    def DFS(self, cur):
        #print '[{}]: Visit {}: {} [{} edges {}]'.format(cur.id, cur.id,
        #        (cur.val if cur.getType() == NodeType.INPUT or
        #            cur.getType() == NodeType.VARIABLE else cur.val.str),
        #        len(cur.edges), '[marked]' if cur.marked else '')
        if cur.marked:
    ...
    def convert_to_functional_graph(self):
        ...
        for cur in self.graph.getUnmarkedNode():
            ...
            if not self.DFS(cur):
                print 'Cannot evaluate, unsolvable dependency graph from node {}'.format(cur.id)
                return False
	...
	type_nodes = [n for n in self.nodes if isinstance(n, TypeNode)]
        #print 'type nodes: {}'.format([(n.id, n.name) for n in type_nodes])
        for t in type_nodes:
            self.n2t[t.name] = t
        rule_nodes = [n for n in self.nodes if isinstance(n, RuleNode)]
        #print 'rule nodes: {}'.format([(n.id, n.name) for n in rule_nodes])
        for r in rule_nodes:
            self.n2r[r.name] = r
        ...
        for s in stmt_nodes:
            if isinstance(s, AssumeNode):
                ...
                if self.dependent:
                    for undecided, (dependents, rtoks) in self.dependent.iteritems():
                ...
                if n_free_variables < n_equations:
                    print 'Overdetermined system, free {}, equation {}'.format(
                            n_free_variables, n_equations)
            else:
                raise ValueError('Unknown stmt: {}'.format(s.toks))

        #print 'Rules: {}'.format([r.name for r in assumed_rules])
        #print 'Symbols: {}'.format(self.var_set)
        #print 'Types: {}'.format(dict([(v, t.name) for v, t in self.v2t.iteritems()]))
        #print 'Equations: {}'.format([(e.str, e.names) for e in self.eq_set])
        #print 'Constraints: {}'.format([e.str for e in self.con_set])
        #print 'Given: {}'.format(self.given)
        ...
        if not iter_vars:
            if self.graph.eval_constraints():
                for tar in self.targets:
                    print 'Result {}: {}'.format(tar, self.v2n[tar].out_val)
        # Handle iterations.
