...
class GraphNode(Node):
    ...
    def dump(self):
        ...
        logging.debug('\t{}'.format(self.getPrintable()))
        logging.debug('\tgiven: {}'.format(self.ordered_given))
        logging.debug('\tproped: {}'.format(self.proped))
    ...
    def clone(self, ext, ext_set):
        ...
        if self.type == NodeType.VARIABLE:
            ...
            dummy_edges = [e for e in node.edges if (node.next(e).getType() == NodeType.VARIABLE and
                node.next(e).exts[-1] != ext) or (node.next(e).getType() == NodeType.INPUT and
    ...
    def removeEdge(self, edge):
        ...
        #print 'Remove {} on {}'.format(edge.getPrintable(), self.id)
...
class Graph(object):
    ...
    def removeNode(self, node):
        ...
        #for e in edges:
        #    print '\t{}'.format(e.getPrintable())
        for e in edges:
            self.removeEdge(e)
        self.node_set.remove(node)
        #print 'Remove [{}]'.format(node.id)
    ...
    def check(self):
        ...
        for n in self.node_set:
            ...
            if has_conflict:
                print 'Conflict detected at node:\n{}'.format(n.getPrintable())
